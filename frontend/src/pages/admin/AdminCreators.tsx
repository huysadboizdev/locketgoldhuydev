...
const CREATOR_IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/heic', 'image/heif']);
  ...
  const onFileChange = (file: File | null) => {
    ...
    if (file) {
      ...
      if ((!CREATOR_IMAGE_TYPES.has(file.type) && !extensionAllowed) || file.size <= 0) {
        setScreenshot(null);
        setPreviewUrl(null);
        setFormError('Ảnh TikTok phải là tệp JPG, PNG, WebP hoặc HEIC hợp lệ.');
        return;
      }
    ...
    <div className="space-y-5">
      ...
      <Modal isOpen={modalOpen} onClose={() => !saving && setModalOpen(false)} title={editing ? 'Cập nhật TikTok KOL' : 'Nâng tài khoản thành TikTok KOL'} maxWidth="2xl">
        <form onSubmit={save} className="space-y-4">
          ...
          <div><label className={labelClass}>Screenshot trang TikTok {editing ? '(để trống nếu giữ ảnh cũ)' : '*'}</label><input type="file" accept=".jpg,.jpeg,.png,.webp,.heic,.heif,image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(e) => onFileChange(e.target.files?.[0] || null)} className="block w-full rounded-xl border border-zinc-300 bg-white p-2.5 text-xs text-zinc-600 file:mr-3 file:rounded-lg file:border-0 file:bg-amber-500 file:px-3 file:py-2 file:font-bold file:text-zinc-950 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-300" /><p className="mt-1.5 text-[11px] text-zinc-500">Hỗ trợ JPG, PNG, WebP, HEIC · tối đa 5 MB. Ảnh gốc không được lưu; hệ thống chỉ giữ phần đã cắt an toàn.</p>
