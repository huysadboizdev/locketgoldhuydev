...
- `locket.locket_api` — `LocketAPI` wraps `getUserByUsername`, `getLastMoment` (against `api.locketcamera.com`), and `restorePurchase` (against `api.revenuecat.com/v1/receipts` with `X-Is-Sandbox: true`). The RevenueCat bearer is read from the `REVENUECAT_PUBLIC_KEY` environment variable. The Locket bearer is the Firebase idToken from `Auth`.
- `locket.gold_cache` — In-memory cache for `_gold_check` results (5-minute TTL). Reduces RevenueCat API load. Only caches successful results; timeouts/errors are never cached (fail-closed).
