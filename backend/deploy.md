...
### 3.2 Tạo user `locket` và thư mục hệ thống

```bash
sudo adduser --system --group --shell /bin/bash --home /opt/locket-gold locket
sudo mkdir -p /var/lib/locket-gold /var/www/locket-gold
sudo chown -R locket:locket /var/lib/locket-gold
sudo chmod 700 /var/lib/locket-gold
```

### 3.3 Clone/Copy Backend + venv

```bash
sudo mkdir -p /opt/locket-gold/backend
# Copy mã nguồn backend vào /opt/locket-gold/backend
...
### 3.4 Tạo file cấu hình môi trường `/etc/locket-gold.env`
...
ADMIN_PASSWORD=<random_password_16_ky_tu>
FLASK_SECRET_KEY=<openssl rand -hex 32>
BEHIND_HTTPS=1
LOCKET_DB=/var/lib/locket-gold/locket.db

# JWT & Refresh Token Knobs
...
```
db: initialized at /var/lib/locket-gold/locket.db
...
```bash
curl -I http://127.0.0.1:5001/api/site-settings
```

### 3.6 Nginx (Cloudflare-fronted hoặc Let's Encrypt)
...
sudo ln -sf /etc/nginx/sites-available/locket-gold /etc/nginx/sites-enabled/locket-gold
