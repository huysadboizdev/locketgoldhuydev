...
class QueueManager:
    ...
    def add_to_queue(self, username, user_id=None, platform=None, plan_id=None, activation_order_id=None):
        """Atomically insert one waiting row per activation order.

        Returns the existing client_id when the same activation order was
        already dispatched, or None when the queue is full.
        """
        conn = db.get_conn()
        clean_platform = platform if platform in ("ios", "android") else None
        try:
            # BEGIN IMMEDIATE serializes dispatchers before the existence check;
            # the partial UNIQUE index in db.init() is the final safety net.
            conn.execute("BEGIN IMMEDIATE")
            if activation_order_id:
            ...
            conn.execute(
                "INSERT INTO queue_requests (client_id, username, status, added_at, user_id, platform, plan_id, activation_order_id) "
                "VALUES (?, ?, 'waiting', ?, ?, ?, ?, ?)",
                (client_id, username, time.time(), user_id, clean_platform, plan_id, activation_order_id),
            )
            if activation_order_id:
                updated = conn.execute(
                    """UPDATE activation_orders
                       SET status = 'queued', queue_client_id = ?, updated_at = ?
                       WHERE id = ? AND status IN ('paid', 'awaiting_queue', 'queued')""",
                    (client_id, time.time(), activation_order_id),
                )
                if updated.rowcount != 1:
                    conn.execute("ROLLBACK")
                    raise ValueError("activation_order_not_dispatchable")
            conn.execute("COMMIT")
        except Exception:
            try:
                conn.execute("ROLLBACK")
        ...
        print(f"Added {username} (user_id={user_id}, platform={clean_platform}, plan_id={plan_id}, order_id={activation_order_id}) to queue with client_id: {client_id}")
        return client_id

    def get_status(self, client_id, user_id=None):
        conn = db.get_conn()
        row = conn.execute(
        ...
        # Queue ownership check: ensure user can only inspect their own requests
        if user_id is not None and row["user_id"] is not None and row["user_id"] != user_id:
            return {
                ...
                "estimated_time": 0,
                "result": None,
                "error": "Yêu cầu không tìm thấy hoặc bạn không có quyền truy cập.",
                "platform": None,
                "plan_id": None,
                "activation_order_id": None,
            }
        ...
        return {
            ...
            "activation_order_id": row_dict.get("activation_order_id"),
        }
    ...
    def _claim_next_waiting(self, slot_id):
            ...
            UPDATE queue_requests
             ...
             WHERE client_id = (
                   SELECT client_id FROM queue_requests
                    ...
                    LIMIT 1)
               AND status='waiting'
            RETURNING client_id, username, activation_order_id
            """,
            (time.time(), slot_id),
        ).fetchone()
        if row is None:
        ...
        if act_id:
            try:
                db.update_activation_order_status(act_id, "processing", queue_client_id=row["client_id"])
            except Exception as e:
                print(f"Failed to set activation order {act_id} to processing: {e}")
    ...
    def _process_request(self, client_id, username, slot_id):
        ...
        try:
            ...
            uid_target = resolve_locket_uid(username)
            ...
            if not uid_target:
                try:
                    ...
                    print(f"getUserByUsername fallback error for {username}: {e}")

            if not uid_target:
                raise Exception(f"Không tìm thấy UID cho tài khoản '{username}'. Vui lòng nhập đúng Username hoặc Link mời Locket.")
            ...
            gold_entitlement = entitlements.get("Gold") or entitlements.get("gold") or {}

            if gold_entitlement.get("product_identifier") in SUBSCRIPTION_IDS or gold_entitlement.get("expires_date"):
                ...
                from .site_settings import get_public_dns_config
                dns_info = get_public_dns_config()

                self._finalize(
                    client_id, slot_id, "completed",
                    result={
                        "success": True,
                        "msg": f"Gói Gold ({product_id}) đã kích hoạt thành công cho @{username}!",
                        "product_identifier": product_id,
                        "expires_date": expires_date,
                        "dns": {
                            "profile_id": dns_info["profile_id"],
                            "apple_url": dns_info["apple_url"],
                            "android_dns": dns_info["hostname"],
                            "hostname": dns_info["hostname"],
                            "doh_url": dns_info.get("doh_url"),
                            "instructions": dns_info.get("instructions"),
                        },
                    },
                )
            else:
                raise Exception(
                    f"Kích hoạt không thành công. Gói Gold chưa được ghi nhận cho {username}."
                )
